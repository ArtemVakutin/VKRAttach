...
public class JwtAuthenticationConverter implements AuthenticationConverter {
    ...
    public Authentication convert(HttpServletRequest request) {
        ...
        if (request.getCookies() != null) {
            return Arrays.stream(request.getCookies()).filter(cookie -> cookie.getName().equals("__Host-auth-token"))
                    .findFirst()
                    .map(cookie -> {
                        Token token = refreshTokenStringDeserializer.apply(cookie.getValue());
                        return new PreAuthenticatedAuthenticationToken(token, token.authorities());
                    }).orElse(null);
        }
        return null;
