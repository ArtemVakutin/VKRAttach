...
public class SandboxSpringSecurityJwtApplication {
    ...
    public UserDetailsService userDetailsService(JdbcTemplate jdbcTemplate) {
        return username -> jdbcTemplate.query("select user_login, user_surname, user_role from vkr_user where user_login = ?",
                (rs, i) -> User.builder()
                        .username(rs.getString("user_login"))
                        .password(rs.getString("user_password"))
                        .authorities(
                                jdbcTemplate.query("select user_role from vkr_user where id_user = ?",
                                        (rs1, i1) ->
